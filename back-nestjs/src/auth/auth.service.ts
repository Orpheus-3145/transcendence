...
export class AuthService {
	...
	async validate(req: Request, res: Response) {
		...
		try {
			...
			this.thrower.throwSessionExcp(`Token validation error: ${error.message}`, `${AuthService.name}.${this.constructor.prototype.validate.name}()`, HttpStatus.UNAUTHORIZED)

		}
		// Check decoded type
		if (typeof decoded !== 'object' || isNaN(Number(decoded.intraId)))
			this.thrower.throwSessionExcp(`Invalid token payload`, `${AuthService.name}.${this.constructor.prototype.validate.name}()`, HttpStatus.UNAUTHORIZED)

		this.logger.debug(`Token [${token}] validated`);
		// Find user
		const user = await this.userService.findOne(Number(decoded.intraId));
		if (!user) 
			this.thrower.throwSessionExcp(`User not found`, `${AuthService.name}.${this.constructor.prototype.validate.name}()`, HttpStatus.NOT_FOUND)
	...
	async logout(res: Response, redir?: string, mess?: string) {
		...
		if (redir)
			...
			responseObj.redirectTo = this.config.get<string>('URL_FRONTEND_LOGIN');
	...
	async getUserAccessToken(code: string): Promise<AccessTokenDTO | null> {
		...
		this.logger.debug(`Fetching access token from Intra42 at URL ${this.config.get<string>('URL_INTRA_TOKEN')}`);
