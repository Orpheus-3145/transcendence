...
export function checkTLSfiles(
    TLSpath: string = SSL_FOLDER, 
    TLSprefix: string = SSL_PREFIX)
  {
...
export function makeTLSfiles(
    TLSpath: string = SSL_FOLDER, 
    TLSprefix: string = SSL_PREFIX): void
  {
  const TLSfileKey: string = `${TLSpath}/${TLSprefix}.key`;
  const TLSfileCrt: string = `${TLSpath}/${TLSprefix}.crt`;
  ...
  // set cert data
  const subject = [
    { name: 'commonName', value: TLSprefix }
  ];
  ...
  if (fs.existsSync(TLSpath) == false)
