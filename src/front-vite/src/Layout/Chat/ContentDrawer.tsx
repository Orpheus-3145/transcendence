...
import { Search as SearchIcon, Settings as SettingsIcon, Logout as LogoutIcon } from '@mui/icons-material';
...
const ContentDrawer: React.FC<ContentDrawerProps> = ({ chatProps, setChatProps }) => {
    ...
    const DrawerContent = (
        ...
        >
            ...
            {chatProps.chatRooms.map((chatRoom) => (
                <Stack key={chatRoom.name.key} direction={'row'} onClick={() => toggleChatStatus(ChatStatus.Chatbox, chatRoom)}
                    sx={{
                        cursor: 'pointer',
    ...
    return (
        <Drawer anchor="right" open={chatProps.chatStatus == ChatStatus.Drawer} onClose={handleClose}
